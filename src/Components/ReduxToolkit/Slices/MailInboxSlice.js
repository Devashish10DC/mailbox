...
export const getMailsFromFirebase = createAsyncThunk(
  ...
  async () => {
    try {
      ...
      const url = `https://mailbox-4990d-default-rtdb.firebaseio.com/${usermail}.json`;
...
export const patchForUpdateReadValue = createAsyncThunk(
  ...
  async (id) => {
    try {
      const url = `https://mailbox-4990d-default-rtdb.firebaseio.com/${usermail}/${id}.json`;
      const res = await fetch(url, {
        method: "PATCH",
        body: JSON.stringify({
          read: true,
...
export const getMailsFromANotherClient = createAsyncThunk(
  ...
  async () => {
    try {
      const url = `https://mailbox-4990d-default-rtdb.firebaseio.com/mails.json`;
      const res = await fetch(url);
      const data = await res.json();
      return data;
    } catch (err) {
